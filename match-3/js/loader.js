...
const BlockNums = 15;
// 消消乐元素
const IMGS = ['9.png', '10.png', '11.png', '12.png', '13.png', '14.png', '15.png', '16.png'].map((img) => `./img/${img}`);
  ...
  isCover() {
    ...
    for (let index = 0; index < allBlock.length; index++) {
      ...
      if (allBlock[index].index === this.index) {
        ...
        // 碰撞逻辑
        const xLeft = target.x;
        const xRight = target.x + target.width;
        const yTop = target.y;
        const yBottom = target.y + target.height;
        //只要thatBlock在这4个临界值内 那么就说明发生了碰撞
        if (
          !(thatBlock.x > xRight || thatBlock.x + thatBlock.width < xLeft || thatBlock.y > yBottom || thatBlock.y + thatBlock.height < yTop)
        ) {
          coverState = true;
          break;
        }
...
function computedBoxPosition(target, targetDomClass) {
  ...
  // 获取元素四周的位置
  storagePosition = storageBox.getBoundingClientRect();
  // 计算StpragePosition的盒子内容的0,0的位置 (盒子的坐标-外部的坐标(app四周的空白) + 边框)
  startLeft = storagePosition.x - AppPosition.x + borderWidth;
  // top 是固定的因为是水平排列都在一条线上
  const top = storagePosition.y - AppPosition.y + borderWidth + 'px';
  ...
  if (!hasBeenStored.length) {
    setStyle(target, { left: `${startLeft}px`, top });
    targetDomClass.left = startLeft;
    // 在最后面叠加直接push
    hasBeenStored.push(Item);
  } else {
    // 查找是否有同样的元素存在
    const hasIndex = hasBeenStored.findIndex((v) => v.targetDomClass.n == targetDomClass.n);
    // 没有同类型的盒子
    if (hasIndex === -1) {
      // 在后面叠加
      const left = startLeft + hasBeenStored.length * targetDomClass.width;
      setStyle(target, { left: `${left}px`, top });
      // 修改绑定的实例链
      ...
      for (let index = hasBeenStored.length - 1; index >= hasIndex; index--) {
        ...
        setStyle(hasBeenStored[index].target, { left: `${newLeft}px` });
  ...
  const removeIndex = allBlock.findIndex((v) => v.index == Item.targetDomClass.index);
...
window.onload = function () {
  ...
  // 给收集盒子加边框
  setStyle(storageBox, {
    border: '8px solid rgb(15, 87, 255)',
  });
};
